...
public class Manager extends Application{
	...
	Setting setting = new Setting(rootpane, InitalizeProfile()); 
	...
	public Profile InitalizeProfile() {
		Profile profile = new Profile(addButtons(rootpane),setting,rootpane); 
		return profile;
	}

	public void start(Stage stage) {
		NewAccount newAcc = new NewAccount(rootpane, InitalizeProfile());
	...
	public HBox addButtons(GridPane grid1) {
		...
		profileBtn.setOnAction(event -> {
			...
			Setting sett = new Setting(grid1, InitalizeProfile());
			Profile prof = new Profile(addButtons(grid1),sett, grid1);
